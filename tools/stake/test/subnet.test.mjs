...
const SEL = { count: selector("getUidCount(uint16)"), hotkey: selector("getHotkey(uint16,uint16)"), permit: selector("getValidatorStatus(uint16,uint16)"), div: selector("getDividends(uint16,uint16)") };

// A fake subnet 7 with 120 uids; hotkey k(uid) = uid + 1000. uid 101 has a permit and 50% dividends.
const SUBNETS = { 7: 120 };
const permitted = new Set([101]);
...
function answer({ params: [{ data }] }) {
  ...
  if (sel === SEL.div) return w(permitted.has(Number(b)) ? 32768 : 0);
...
const { findOnSubnet, subnetHotkeys, getUidCount } = await import("../src/bittensor.js");
