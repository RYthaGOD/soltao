...
// commit 3b5609f of 3 Sep 2026, read 24 Sep: a Balances.Transfer to the address; below 0.01 TAO it is ignored as dust).
//
...
// batch's own ExtrinsicSuccess/ExtrinsicFailed event, or, if the node no longer has it, by the coldkey's
// free balance read before and after.
...
export async function runPayment({
  ...
  const outcome = async () => {
    const [freeAfter, event] = await Promise.all([ops.free(coldkey), dispatchResult(ops, saved.rec)]);
    // The batch's own success event decides. Without it (old blocks pruned), free TAO falling by at least
    // the amount is the fallback, which a transfer arriving at the same time could hide.
    const landed = saved.rec?.status === "included" && (event ? event === "success" : BigInt(saved.freeBefore) - freeAfter >= BigInt(saved.amount));
